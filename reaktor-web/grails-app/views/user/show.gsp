	...
	<header>
		...
		<section>
			...
			<div class="message" role="status">${flash.message}</div>
				...
				<div class="row">
					<g:if test="${userInstance?.reactions}">
						<div class="col-md-3 col-md-push-3 col-size-med">Reactions:</div>
						<g:each in="${userInstance.reactions}" var="r">
								<div class="row">
									<div class="col-md-3 col-md-offset-6 col-size-med">
										<span class="text-info" style="font-size:14px">
											<g:link controller="reaction" action="show" id="${r?.id }">${r?.encodeAsHTML()}</g:link>
										</span>
									</div>
								</div>
						</g:each>
